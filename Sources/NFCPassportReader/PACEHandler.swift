...
public class PACEHandler {
    ...
    public static let CAN_PACE_KEY_REFERENCE : UInt8 = 0x02 // Not currently supported
    public static let PIN_PACE_KEY_REFERENCE : UInt8 = 0x03 // Not currently supported
    public static let CUK_PACE_KEY_REFERENCE : UInt8 = 0x04 // Not currently supported
    ...
    // Params used
    public var paceKey : [UInt8] = []
    public var paceKeyType : UInt8 = 0
    public var paceOID : String = ""
    public var parameterSpec : Int32 = -1
    public var mappingType : PACEMappingType!
    public var agreementAlg : String = ""
    public var cipherAlg : String = ""
    public var digestAlg : String = ""
    public var keyLength : Int = -1
...
*/
    ...
    /// Generates an ephemeral public/public key pair based on mapping parameters from step 2, and then sends
...
///         - Tuple of Generated Ephemeral KeyPair and the Passport's public key
    ...
    /// Here the shared secret is computed from our ephemeral public key and the passports ephemeral public key
...
extension PACEHandler {
    ...
    func doECDHMappingAgreement( mappingKey : OpaquePointer, passportPublicKeyData: [UInt8], nonce: OpaquePointer ) throws -> OpaquePointer {
        ...
        guard let sharedSecretMappingPoint = self.computeECDHMappingKeyPoint(publicKey: mappingKey, inputKey: passportPublicKeyData) else {
            // Error
    ...
    /// Performs the ECDH PACE GM key agreement protocol by multiplying a public key with a public key
    /// - Parameters:
    ///   - key: an EVP_PKEY structure containng a ECDH public key
    ///   - inputKey: a public key
    /// - Returns: a new EC_POINT
    func computeECDHMappingKeyPoint( publicKey : OpaquePointer, inputKey : [UInt8] ) -> OpaquePointer? {
        
        let ecdh = EVP_PKEY_get1_EC_KEY(publicKey)
        defer { EC_KEY_free(ecdh) }

        let publicECKey = EC_KEY_get0_public_key(ecdh) // BIGNUM
        ...
        // Multiply our public key with the passports public key to get a new point
        EC_POINT_mul(group, output, nil, ecp, publicECKey, nil)
